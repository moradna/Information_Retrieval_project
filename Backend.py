...
class Backend_Search:
    ...
    def get_candidate_doc_for_title_and_anchor(self, query, index, folder_name):  # לבדוק אם זה גם לאנקור
        """
    ...
    def get_candidate_doc_for_body(self, query, index, query_score):
        """
    ...
    def search_title(self, query, pre=False):
        ...
        scores_dict = self.get_candidate_doc_for_title_and_anchor(query, index_title, folder_name)
    ...
    def search_body(self, query, pre=False):
        ...
        if len(query_score) == 0:
            return []
        candidate_docs = self.get_candidate_doc_for_body(query, index, query_score)
    ...
    def backend_search_helper(self, processed_query, index, folder):
        ...
        candidate_docs = self.get_candidate_doc_for_bm2_body(processed_query, index, query_idf, folder)
        if candidate_docs is None:
            return []
        res = []
        heap = []
    ...
    def get_candidate_doc_for_bm2_body(self, processed_query, index, query_idf, folder_name):
        """
        :param folder_name: name of the folder in the bucket with the bins
